...
\end{equation}
Where  is the VG value at the time ,  is the AutoRegression (AR) coefficient, and  is the Moving Average (MA) coefficient. This model is denoted as arima(p,0,q).\\
...
<<dataFilesN, echo=FALSE, fig=FALSE, eval=FALSE>>=
setwd("/home/albert/simplifiedthesis")
filezN1 <- list.files(path=".", pattern="Negative_comb_1")
filezN3 <- list.files(path=".", pattern="Negative_comb_3")
filezN5 <- list.files(path=".", pattern="Negative_comb_5")
filezN7 <- list.files(path=".", pattern="Negative_comb_7")
DatN1 <- read.csv(filezN1[3])
DatN1 <- DatN1[which(DatN1[,2] < 10 & DatN1[,2] > -10), ]
DatN1 <- ts(DatN1)
DatN3 <- read.csv(filezN3[3])
DatN3 <- DatN3[which(DatN3[,2] < 10 & DatN3[,2] > -10), ]
DatN3 <- ts(DatN3)
DatN5 <- read.csv(filezN5[3])
DatN5 <- DatN5[which(DatN5[,2] < 10 & DatN5[,2] > -10), ]
DatN5 <- ts(DatN5)
DatN7 <- read.csv(filezN7[3])
...
<<dataFilesN>>
par(mfrow=c(2,2))
plot(DatN1[,2], xlab = "Data Points",
                ylab = "Variable Geometry (VG) Value", col="blue")
lines(rep(0,length(DatN1[,2])),col="green")
plot(DatN3[,2], xlab = "Data Points",
                ylab = "", col="blue")
lines(rep(0,length(DatN3[,2])),col="green")
plot(DatN5[,2], xlab="Data Points",
                ylab = "Variable Geometry (VG) Value", col="blue")
lines(rep(0,length(DatN5[,2])),col="green")
plot(DatN7[,2], xlab="Data Points",
                ylab = "", col="blue")
lines(rep(0,length(DatN7[,2])),col="green")
@
...
<<dataFilesP, echo=FALSE, fig=TRUE, eval=FALSE>>=
setwd("/home/albert/simplifiedthesis")
filezP1 <- list.files(path=".", pattern="Positive_comb_1")
filezP3 <- list.files(path=".", pattern="Positive_comb_3")
filezP5 <- list.files(path=".", pattern="Positive_comb_5")
filezP7 <- list.files(path=".", pattern="Positive_comb_7")
DatP1 <- read.csv(filezP1[3])
DatP1 <- DatP1[which(DatP1[,2] < 10 & DatP1[,2] > -10), ]
DatP1 <- ts(DatP1)
DatP3 <- read.csv(filezP3[3])
DatP3 <- DatP3[which(DatP3[,2] < 10 & DatP3[,2] > -10), ]
DatP3 <- ts(DatP3)
DatP5 <- read.csv(filezP5[3])
DatP5 <- DatP5[which(DatP5[,2] < 10 & DatP5[,2] > -10), ]
DatP5 <- ts(DatP5)
DatP7 <- read.csv(filezP7[3])
...
<<dataFilesP>>
par(mfrow=c(2,2))
plot(DatP1[,2], xlab = "Data Points",
                ylab = "Variable Geometry (VG) Value", col="red")
lines(rep(0,length(DatP1[,2])),col="green")
plot(DatP3[,2], xlab = "Data Points)",
                ylab = "", col="red")
lines(rep(0,length(DatP3[,2])),col="green")
plot(DatP5[,2], xlab="Data Points",
                ylab = "Variable Geometry (VG) Value", col="red")
lines(rep(0,length(DatP5[,2])),col="green")
plot(DatP7[,2], xlab="Data Points",
                ylab = "", col="red")
lines(rep(0,length(DatP7[,2])),col="green")
@
...
lines(rep(0,length(DatN7[,2])),col="green")
@
\caption{First order differenced VG Value calculation demonstration, for those data set WITHOUT compressor stall fault event in next flight. Up-Left - 1 Combined Flight Data; Up-Right - 3 Combined Flight Data; Down-Left - 5 Combined Flight Data; Down-Right - 7 Combined Flight Data.}
...
lines(rep(0,length(DatP7[,2])),col="green")
@
\caption{First order differenced VG Value calculation demonstration, for those data set WITH compressor stall fault event in next flight. Up-Left - 1 Combined Flight Data; Up-Right - 3 Combined Flight Data; Down-Left - 5 Combined Flight Data; Down-Right - 7 Combined Flight Data.}
...
First, check the  parameter by using the R code: \\
Where  stands for the maximum likelihood estimate (MLE) of the innovations variance. The difference between the expected mean at time t, given the time series prior to t, and the actual value is called the innovation. Measuring the variance of the innovation will give you a better idea of how "noisy" the process is.\\
Second, check the  parameter by using R code:\\
Where  stands for a logarithm of likelihood function. In statistics, a Likelihoodfunction (often simply the Likelihood) is a function of the parameters of a statistical model. The likelihood of a set of parameter values, $\theta$, given outcomes x, is equal to the probability of those observed outcomes given those parameter values, that is:\\
Third, check the  parameter by using R code:\\
Where  stands for Akaike Information Criterion, which is a measure of the relative quality of a statistical model for a given set of data.\\
Fourth, check the Percentage of Significant Coefficients () parameter.\\
represents the percentage of significant coefficients among the AR coefficients and MA coefficients of the fitted arima(p,0,q) model.  Let  be the first order differenced ts data of VG,  be the fitted arima(p,0,q) model. That is:\\
If one of the coefficients of the fitted arima(p,0,q) model  (k  = 1, 2, … (p+q)) meet following condition, it is considered as significant.\\
In R programming environment,  is the coefficients of the resulting arima(p,0,q) model, where  be AR coefficients while k = 1, 2, … p, and  be MA coefficients while k = (p+1), (p+2), … (p+q). Correspondingly,  be the estimated variance of coefficient.\\
Therefore, the  is calculated as:\\
...
For example, process the sample data of Figure 18, is fit to an arima(12,0,4) model for the first order of VG data, by use of an R script, the  of arima(12,0,4) is calculated as
(-0.1815, 0.0002387, 0.01500, 0.03247, 0.02954, -0.002241, 0.05759, 0.04050, 0.006447, 0.01128, 0.01244, 0.02152, -0.05026, 0.04650, 0.02096, -0.01320)\\
...
And the  of arima(12,0,4) is calculated as\\
(0.0003720, -0.0002028, -0.00002361, -0.0002780, 0.00001439, 0.00003668, 0.00005502, 0.00005025, 0.00005391, 0.00006573, 0.00003563, 0.00005364, -0.0003380, 0.0003338, -0.00001389, 0.0003176)\\
Thus, the ratio of coefficient to variance of coefficient  is calculated as
(487.84, 1.18, 635.56, 116.78, 2053.07, 61.09, 1046.79, 805.91, 119.60, 171.66, 349.30, 401.21, 148.69, 139.29, 1508.98, 41.57)\\
The second AR coefficient (AR2) is not significant because its ratio of coefficient to variance of coefficient valued as of 1.18. The rest of the coefficients of arima(12,0,4) model are significant. Therefore the  can be calculated as\\
...
\item Use a Fast Fourier Transform to achieve higher computation efficiency.  Investigate other perspectives to achieve better models.
